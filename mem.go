...
func sizeOf(v reflect.Value, cache map[uintptr]bool) int32 {
	switch v.Kind() {
	case reflect.Array:
		fallthrough
	case reflect.Slice:
	...
	case reflect.Struct:
		...
		return sum
	case reflect.String:
		return int32(len(v.String()) + int(v.Type().Size()))
	case reflect.Ptr:
		// return Ptr size if this node has been visited already (infinite recursion)
		if cache[v.Pointer()] {
			return int32(v.Type().Size())
		}
		cache[v.Pointer()] = true
		if v.IsNil() {
			return int32(reflect.New(v.Type()).Type().Size())
		}
		s := sizeOf(reflect.Indirect(v), cache)
		if s < 0 {
			return -1
		}
		return s + int32(v.Type().Size())
	case reflect.Bool,
	...
	case reflect.Map:
		...
		return sum + int32(v.Type().Size()) + int32(float64(len(keys))*10.79)
	case reflect.Interface:
		return sizeOf(v.Elem(), cache) + int32(v.Type().Size())
	}
	return -1
}
